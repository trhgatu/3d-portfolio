        ...
        <div className="journey-path-container relative max-w-4xl mx-auto">
          ...
          <div className="flex flex-col gap-32 md:gap-48 pb-32">
            {MILESTONES.map((item) => (
              <div
                key={item.id}
                className={`milestone-stone relative flex items-center w-full ${
                  item.side === "left" ? "justify-end md:pr-12" : "justify-start md:pl-12"
              ...
              >
                ...
                >
                  ...
                  <h3 className="font-cinzel-decorative text-2xl text-white mb-2 group-hover:text-white/90 transition-colors">
                    {item.title}
                  </h3>
                ...
                {/* Center Node on Line */}
                <div
                  className={`absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20 w-4 h-4 bg-black border-2 border-white rotate-45 group-hover:scale-125 transition-transform duration-300 shadow-[0_0_10px_white]`}
                />
              </div>
            ))}
          </div>
