  ...
  >
    <div className={`relative ${featured ? "h-64" : "h-48"}`}>
      ...
      {featured && (
        <div className="absolute top-4 right-4">
          <span className="px-2 py-1 bg-orange-600 text-white text-xs rounded-full">Featured</span>
        </div>
      )}
    ...
    <div className={featured ? "p-6" : "p-4"}>
      <div className="flex items-center gap-2 mb-2">
        <h3 className={`font-bold font-serif text-white ${featured ? "text-xl" : ""}`}>
          {foundation.name}
        </h3>
      ...
      <p className="text-gray-300 font-serif text-sm mb-3">{foundation.description}</p>
