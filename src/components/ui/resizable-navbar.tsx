...
interface NavbarProps extends React.ComponentProps<typeof motion.div> {
...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
  ...
  return (
    <motion.div
      ...
      className={cn(
        "absolute inset-0 hidden flex-1 flex-row items-center justify-center space-x-2 text-sm font-medium font-mono text-zinc-600 transition duration-200 hover:text-zinc-800 lg:flex lg:space-x-2",
        className
      )}
    >
      {items.map((item, idx) => (
        ...
        >
          ...
          <span className="relative nav-anim z-20 inline-block min-w-[80px] text-center whitespace-nowrap">
            {item.name}
          </span>
        </a>
      ))}
